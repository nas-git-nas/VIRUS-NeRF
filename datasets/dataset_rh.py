...
class DatasetRH(DatasetBase):
    ...
    def __init__(
        ...
        poses, rgbs, depths_dict, sensors_dict, stack_ids, times = self.readMeta(
            df=self.df,
            img_wh=img_wh,
            cam_ids=cam_ids,
    ...
    def _convertDepthImgs(
        ...
        for cam_id, directions in directions_dict.items():
            sensor_mask = (int(cam_id[-1]) == stack_id) # (N,)
