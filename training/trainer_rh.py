...
class TrainerRH(Trainer):
    ...
    def evaluate(self):
        ...
        print(
            f"evaluation: " \
            + f"psnr_avg={metrics_dict['psnr']} | " \
            + f"ssim_avg={metrics_dict['ssim']} | " \
            + f"depth_mae={metrics_dict['mae']} | " \
            + f"depth_mare={metrics_dict['mare']} | " \
            + f"depth_mnn={metrics_dict['mnn']} | " \
        )
    ...
    def evaluatePlot(
        ...
        """
        N = self.args.eval.num_depth_pts
        ...
        if self.args.eval.num_plot_pts > self.args.eval.num_depth_pts:
            print(f"WARNING: trainer_RH.evaluatePlot(): num_plot_pts={self.args.eval.num_plot_pts} "
                  f"should be smaller or equal than num_depth_pts={self.args.eval.num_depth_pts}")
            self.args.eval.num_plot_pts = self.args.eval.num_depth_pts
        elif self.args.eval.num_plot_pts < self.args.eval.num_depth_pts:
    ...
    def _evaluateStep(
        ...
        self.logs['color_loss'].append(color_loss.item())
        self.logs['depth_loss'].append(depth_loss.item())
